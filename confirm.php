							...
							if(mysql_num_rows($r10)>0) {
								while($row10 = mysql_fetch_assoc($r10)) {
									...
									if($balance>=$etotr) {
										...
										$q11 = "SELECT `e_seats_left` FROM `ride_search` WHERE `fno`='$to'";
										$r11 = mysql_query($q11);
										if($r11) {
											if(mysql_num_rows($r11)>0) {
												while($row11 = mysql_fetch_assoc($r11)) {
													...
													mysql_query("UPDATE `ride_search` SET `e_seats_left`='$dec_seats11' WHERE `fno`='$to'");
												}				
											} 
											date_default_timezone_set('CBD/Westie');
											$a1 = date('H:i:s');
											$e1 = str_replace(':', '', $a1);
											$a2 = date('d-m-Y');
											$e2 = str_replace('-', '', $a2);
										...
										$q12 = "SELECT `e_seats_left` FROM `ride_search` WHERE `fno`='$fro'";
										$r12 = mysql_query($q12);
										if($r12) {
											if(mysql_num_rows($r12)>0) {
												while($row12 = mysql_fetch_assoc($r12)) {
													...
													mysql_query("UPDATE `ride_search` SET `e_seats_left`='$dec_seats12' WHERE `fno`='$fro'");
												}				
											} 
											date_default_timezone_set('CBD/Westie');
											$a5 = date('H:i:s');
