        ...
        <div class="col-lg-8">
          <div class="well bs-component">
            ...
            if(isset($_GET['path'])===true 
              ...
              if($path==='oneway') {
              echo '<legend>Rides from '.$from.' to '.$to.'</legend>';
              $query = "SELECT * FROM `ride_search` WHERE `from_city`= '$from' AND `to_city` = '$to' AND `departure_date` = '$departdate'";
              ...
              <center><button type="submit" id="class" name="class" value="<?php echo $class; ?>" class="btn btn-primary">Choose Rides</button></center>
              <?php
            } else { echo 'No Rides found';}
          }
              else {  die(mysql_error()); }
          } 
          else if($path==='return') {
            echo '<legend>Rides from '.$from.' to '.$to.'</legend>';
              $query1 = "SELECT * FROM `ride_search` WHERE `from_city`= '$from' AND `to_city` = '$to' AND `departure_date` = '$departdate'";
              ...
              echo '<legend>Rides from '.$to.' to '.$from.'</legend>';
              if(isset($_GET['return_date'])===true) {
                $returndate = $_GET['return_date'];
              $query2 = "SELECT * FROM `ride_search` WHERE `from_city`= '$to' AND `to_city` = '$from' AND `departure_date` = '$returndate'";
              ...
              <input type="hidden" name="count_a" value="<?php echo $counta; ?>"/>
              <input type="hidden" name="count_c" value="<?php echo $countc; ?>"/>
              <center><button type="submit" id="class" value="<?php echo $class; ?>" name="class" class="btn btn-primary">Choose Rides</button></center>
              <?php
            } else { echo 'No Rides found';}
          }
              else {  die(mysql_error()); }
              ...
              <h3>Services offered by our Line</h3>
              <h5>Travel across all cites in Kenya</h5>
              <h5>Name of Driver: <strong> Swift</strong></h5>
