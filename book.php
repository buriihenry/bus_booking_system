			...
			<div class="well bs-component">
				<?php 
					if(f_logged_in()===true) {
						...
						$to = $_GET['chose_to'];
						$fro = $_GET['chose_fro'];
						$qu1 = "SELECT * FROM `ride_search` WHERE `fno`='$to'";
						$ru1 = mysql_query($qu1);
						while($ro1 = mysql_fetch_assoc($ru1)) {
							$arr1 = $ro1['arrival_time'];
						}
						$qu2 = "SELECT * FROM `ride_search` WHERE `fno`='$fro'";
						$ru2 = mysql_query($qu2);
							...
							if(isset($_GET['chose_to'])===true) {
								if(isset($_GET['chose_fro'])===true) {
									...
									$fro = $_GET['chose_fro'];
										...
										$countc = $_GET['count_c'];
										$class = $_GET['class'];
										$q1 = "SELECT * FROM `ride_search` WHERE `fno`='$to'";
										$r1 = mysql_query($q1);
										while($row1 = mysql_fetch_assoc($r1)) {
											echo '<legend>Your ride from '.$row1['from_city'].' to '.$row1['to_city'].'</legend>';
											echo 'ride Number: '.$row1['fno'].'<br>';
											if($class==='Economy') {
												if(($counta+$countc)<=$row1['e_seats_left']) {
													...
													echo 'Not enough available seats. Sorry, please check some other Ride!';
										...
										$q2 = "SELECT * FROM `ride_search` WHERE `fno`='$fro'";
										$r2 = mysql_query($q2);
										while($row2 = mysql_fetch_assoc($r2)) {
											echo '<legend>Your ride from '.$row2['from_city'].' to '.$row2['to_city'].'</legend>';
											echo 'ride Number: '.$row2['fno'].'<br>';
											if($class==='Economy') {
						...
						} else { echo 'Sorry, but it appears your return ride takes off before you land at your destination! Please select another ride!';
			...
			<div class="well bs-component">
				<form class="form-horizontal" action="card.php" method="GET">
		            ...
		            <input type="hidden" name="count_a" value="<?php echo $counta; ?>"/>
              		...
              		?>
		            <center><button type="submit" id="book_f" name="book_f" class="btn btn-primary">Book ride</button></center>
				</form>
			</div>
		</div>
