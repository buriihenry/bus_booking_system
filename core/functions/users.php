<?php
	...
	function f_activate($f_mailid, $f_mailcode) {
		...
		if(mysql_result(mysql_query("SELECT COUNT(`f_id`) FROM `ride_users` WHERE `f_mailid` = '$f_mailid' AND `f_mailcode` = '$f_mailcode' AND `f_active` = 0"), 0) == 1) {
			mysql_query("UPDATE `ride_users` SET `f_active` = 1 WHERE `f_mailid` = '$f_mailid' ");
			return true;
		}
		else {
			return false;
	...
	function update_f($f_id, $update_data) {
		...
		mysql_query("UPDATE `ride_users` SET " . implode(', ',$update) . "WHERE `f_id` = $f_id") or die(mysql_error());
	}

	function f_change_password($f_id, $f_password) {
		$f_id = (int)$f_id;
		$f_password = md5($f_password);
		mysql_query("UPDATE `ride_users` SET `f_password` = '$f_password', `f_passrec` = 0 WHERE `f_id` = $f_id");
	}
	...
	function f_data($f_id){
		...
		if($func_num_args > 1) {
			...
			$data = mysql_fetch_assoc(mysql_query("SELECT $fields FROM `ride_users` WHERE `f_id` = $f_id"));			
			return $data;
		}
 	}
	...
	function f_exists($f_uname) {
		...
		$query = mysql_query("SELECT COUNT(`f_id`) FROM `ride_users` WHERE `f_uname`= '$f_uname'");
		return (mysql_result($query, 0) == 1) ? true : false; 
	}

	function f_email_exists($f_mailid) {
		$f_mailid = sanitize($f_mailid);
		$query = mysql_query("SELECT COUNT(`f_id`) FROM `ride_users` WHERE `f_mailid`= '$f_mailid'");
		return (mysql_result($query, 0) == 1) ? true : false; 
	}
	
	function f_regid_exists($f_regid) {
		$f_regid = sanitize($f_regid);
		$query = mysql_query("SELECT COUNT(`f_id`) FROM `ride_users` WHERE `f_regid`= '$f_regid'");
		return (mysql_result($query, 0) == 1) ? true : false; 
	}

	function f_active($f_uname) {
		$f_uname = sanitize($f_uname);
		$query = mysql_query("SELECT COUNT(`f_id`) FROM `ride_users` WHERE `f_uname`= '$f_uname' AND `f_active` = 1");
		return (mysql_result($query, 0) == 1) ? true : false; 
	}

	function f_id_from_username($f_uname) {
		$f_uname = sanitize($f_uname);
		$query = mysql_query("SELECT `f_id` FROM `ride_users` WHERE `f_uname` = '$f_uname'");
		return mysql_result($query, 0, 'f_id');
	} 
	
	function f_id_from_email($f_mailid) {
		$f_mailid = sanitize($f_mailid);
		$query = mysql_query("SELECT `f_id` FROM `ride_users` WHERE `f_mailid` = '$f_mailid'");
		return mysql_result($query, 0, 'f_id');
	} 
	...
	function f_login($f_uname, $f_password) {
		...
		$f_uname = sanitize($f_uname);
		$f_password = md5($f_password);
		$query = mysql_query("SELECT COUNT(`f_id`) FROM `ride_users` WHERE `f_uname`= '$f_uname' AND `f_password` = '$f_password'");
